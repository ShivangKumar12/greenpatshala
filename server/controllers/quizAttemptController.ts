...
import { quiz_attempts, quizzes, questions, users } from '../../shared/schema';
...
export async function submitQuizAttempt(req: Request, res: Response): Promise<Response> {
  try {
    ...
    if (!quiz.is_free) {
      const [access] = await db.select().from(sql.raw('quiz_access')).where(and(
        sql.raw('user_id = ?', [userId]),
        sql.raw('quiz_id = ?', [quizId])
      ));
      if (!access) {
    ...
    // ✅ FIXED: Check attempts count instead of just existence
    ...
    // ✅ ONLY block if attempts exhausted
    if (attemptCount >= attemptsAllowed) {
      return res.status(400).json({ 
        success: false, 
        message: `You have used all ${attemptsAllowed} attempt(s) for this quiz. Please wait or try another quiz.` 
      });
    }
    ...
    const [result] = await db.insert(quiz_attempts).values({
      user_id: userId,
      quiz_id: quizId,
      score,
    ...
    if (quiz.results_declared) {
      return res.status(201).json({
        ...
        result: {
          ...
          timeTaken,
          answers: quiz.show_results ? evaluatedAnswers : null,
...
export async function getQuizStatistics(req: Request, res: Response): Promise<Response> {
  try {
    ...
    return res
      .status(500)
      .json({ success: false, message: 'Failed to fetch statistics', error: error.message });
  }
}
