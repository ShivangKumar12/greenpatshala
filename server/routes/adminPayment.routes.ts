// server/routes/adminPayment.routes.ts - INLINE SOLUTION
...
router.get('/all', authenticate, adminAuth, async (req: Request, res: Response) => {
  try {
...
router.get('/stats', authenticate, adminAuth, async (req: Request, res: Response) => {
  try {
...
router.delete('/:id', authenticate, adminAuth, async (req: Request, res: Response) => {
  try {
    ...
    console.log('[ADMIN - DELETE PAYMENT] ID:', paymentId);
    ...
    console.log('[ADMIN - DELETE PAYMENT] Success, ID:', paymentId);
    ...
    console.error('[ADMIN - DELETE PAYMENT ERROR]', {
      message: error.message,
      stack: error.stack,
    });
    return res.status(500).json({
